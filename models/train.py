...
def train(args, data):
    ...
    for i, batch in enumerate(iterator):
        ...
        if (i + 1) % args.print_freq == 0:
            ...
            print(f'train loss: {loss:.3f} / dev loss: {dev_loss:.3f} / test loss: {test_loss:.3f} / dev acc: {dev_acc:.3f} / test acc: {test_acc:.3f}')
...
def main():
    ...
    parser.add_argument('--max-sent-len', default=-1, type=int, help='max length of input sentences model can accept, if -1, it accepts any length')
    ...
    if args.data_type == 'SNLI': # TODO: change dataset
        ...
        raise NotImplementedError('only SNLI or Quora is possible') # TODO: change the error
    ...
    torch.save(best_model.state_dict(), f'saved_models/BIMPM_{args.data_type}_{args.model_time}.pt')
